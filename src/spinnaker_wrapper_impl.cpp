...
std::string SpinnakerWrapperImpl::setDouble(
...
std::string SpinnakerWrapperImpl::setBool(
  const std::string & nn, bool val, bool * retVal)
{
  *retVal = !val;
  return (
...
std::string SpinnakerWrapperImpl::setInt(
  const std::string & nn, int val, int * retVal)
{
  *retVal = -1;
  return (
...
bool SpinnakerWrapperImpl::startCamera(const SpinnakerWrapper::Callback & cb)
{
  ...
  if (set_acquisition_mode_continuous(nodeMap)) {
    ...
    thread_ =
      std::make_shared<std::thread>(&SpinnakerWrapperImpl::monitorStatus, this);
    cameraRunning_ = true;
