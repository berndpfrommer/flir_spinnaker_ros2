...
{
  wrapperImpl_.reset(new SpinnakerWrapperImpl());
}
...
void SpinnakerWrapper::refreshCameraList()
{
  wrapperImpl_->refreshCameraList();
}
...
bool SpinnakerWrapper::startCamera(const Callback & cb)
{
  return wrapperImpl_->startCamera(cb);
}

bool SpinnakerWrapper::stopCamera() { return wrapperImpl_->stopCamera(); }

std::string SpinnakerWrapper::getPixelFormat() const
{
  return wrapperImpl_->getPixelFormat();
}
...
std::string SpinnakerWrapper::getNodeMapAsString()
{
  return (wrapperImpl_->getNodeMapAsString());
}
...
std::string SpinnakerWrapper::setDouble(
...
std::string SpinnakerWrapper::setBool(
...
std::string SpinnakerWrapper::setInt(
  const std::string & nodeName, int val, int * retVal)
{
  try {
...
void SpinnakerWrapper::setComputeBrightness(bool b)
{
  wrapperImpl_->setComputeBrightness(b);
}

void SpinnakerWrapper::setAcquisitionTimeout(double t)
{
  wrapperImpl_->setAcquisitionTimeout(t);
}
